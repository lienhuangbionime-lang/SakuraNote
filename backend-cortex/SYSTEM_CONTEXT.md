...
> **Verification Status**: ✅ [2026-02-17] v3.5 Upgrade Planning Complete. Three Pillars defined.
...
**Status**: v3.5 Upgrade in Progress (RAG + Chat Memory + Automation)
**Lead Developer**: Cortex (AI) + 蒼禾 (Commander)

---

## 🚀 v3.5 Development Workflow (MANDATORY FOR ALL AI)

### Phase 1: RAG Infrastructure (CURRENT)
**Status**: ⏳ Ready to Execute  
**Owner**: Next AI Developer  
**Tasks**:
1. Implement `app/services/embedder.py`
   - Use `gemini-embedding-001` (3072 dimensions)
   - Add retry logic and error handling
2. Modify `app/api/v1/ingest.py`
   - Call `embedder.generate_embedding()` before Supabase insert
   - Update `embedding` field in memories table
3. Implement `app/services/rag.py`
   - Hybrid search: semantic (pgvector) + filters (date, tags)
   - Use SQL function `match_memories(query_embedding, threshold, count)`

**Verification**:
- [ ] New memory auto-generates embedding
- [ ] Search "投資策略" returns relevant memories
- [ ] Hybrid filters work (date + semantic)

---

### Phase 2: Cognitive Awakening (NEXT)
**Status**: 📋 Planned  
**Owner**: AI Developer after Phase 1  
**Tasks**:
1. Create `prompts/system_cortex.md`
   - Conversational tone (vs. structured `system_daily.md`)
   - Glass Box philosophy
2. Modify `app/api/v1/chat.py`
   - Inject RAG context before generation
   - Format: `## Relevant Context\n{top 5 memories}`

**Verification**:
- [ ] Chat references past memories
- [ ] SSE streaming smooth
- [ ] Distinguishes "write diary" vs "chat" modes

---

### Phase 3: Agentic Automation (FINAL)
**Status**: 📋 Planned  
**Owner**: AI Developer after Phase 2  
**Tasks**:
1. Implement `app/subconscious/daily_reflection.py`
   - Query today's memories
   - Generate summary with Gemini Pro
   - Store as `category="ai_reflection"`
2. Modify `main.py`
   - Add APScheduler lifespan manager
   - Schedule: `daily_reflection` (23:00), `twse_scanner` (14:30)

**Verification**:
- [ ] Reflection runs at 23:00 without errors
- [ ] TWSE scanner runs at 14:30
...
### 0. Google Drive Sync Protocol (MANDATORY)
```
Every Modification → Run Sync Tool → G:\我的雲端硬碟\Cortex (Single Source of Truth)
```

**CRITICAL RULE**: After ANY modification to the following files, you MUST run:
```powershell
...
- `evolution_log.json`
- `.cursorrules`
- Any file in `prompts/`, `schemas/`, `migrations/`, `tools/`
- Core backend files: `main.py`, `app/core/*`, `app/api/v1/*`, `app/workers/*`

**Verification**: Check `G:\我的雲端硬碟\Cortex\.sync_metadata.json` for last sync time.

**Rationale**: Google Drive is the persistent backup. Local files may be lost during system crashes or reinstalls. The sync tool ensures all critical knowledge is preserved.
...
- ❌ **NEVER** use table name `LogEntry` (This is a legacy artifact).
- ✅ **ALWAYS** use table name `memories` for daily log data.
- ✅ **ALWAYS** use `LogEntrySchema` as the Pydantic class name (to maintain UI compatibility) but map it to the `memories` table.
