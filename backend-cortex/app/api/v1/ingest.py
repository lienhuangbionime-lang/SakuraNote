...
async def ingest_log(request: IngestRequest):
    ...
    try:
        ...
        model_name = model_config.get("model", "gemini-flash-lite-latest")
...
"""
        ...
        import uuid
        embedding = get_embeddings(ai_data.get("markdown_body") or request.content)

        db_payload = {
            "id": str(uuid.uuid4()),
            "date": ingest_date,
            "content": ai_data.get("markdown_body") or request.content,
            "mood": mood,
            "focus": focus,
            "energy": energy,
        ...
        # 4. 寫入 Supabase (含重複日期處理)
        if supabase:
            try:
                # 為了防止重複，先嘗試更新（即使 SQL 還沒設 UNIQUE，我們在邏輯上先對齊日期）
                # 這樣同一個日期的多筆輸入會合併（或覆蓋）而非重複
                ...
                if tasks_list:
                    ...
                    # If target_id wasn't set (because we just inserted), we need to fetch it?
                    # Actually, for 'insert', supabase-py returns the inserted data if we ask nicely, 
                    # but here we didn't capture it.
                    # Workaround: We query by date again or trust 'target_id' logic above.
                    # The logic above:
                    # if check_res.data -> target_id = ...
                    # else -> insert ... (we missed capturing ID here)
