...
async def get_projects():
    """List all projects."""
    res = supabase.table("Project").select("*").order("updated_at", desc=True).execute()
    return res.data
...
async def update_project(project_id: str, project: ProjectUpdate):
    ...
    res = supabase.table("Project").update(data).eq("id", project_id).execute()
    if not res.data:
...
async def delete_project(project_id: str):
    """Delete a project."""
    res = supabase.table("Project").delete().eq("id", project_id).execute()
...
async def merge_project(project_id: str, merge_req: ProjectMergeRequest):
    ...
    source_res = supabase.table("Project").select("*").eq("id", project_id).execute()
    if not source_res.data:
        raise HTTPException(status_code=404, detail="Source project not found")
    source_project = source_res.data[0]
    
    # 2. Get Target Project
    target_res = supabase.table("Project").select("*").eq("id", merge_req.target_project_id).execute()
    ...
    # Delete Source
    supabase.table("Project").delete().eq("id", project_id).execute()
