...
async def get_available_models():
    ...
    try:
        import google.generativeai as genai
        # Read key directly from env to avoid circular imports if any
        ...
        # Sort models: Pro > Flash > Others
        models.sort(key=lambda x: (
            0 if "pro" in x.id and "exp" not in x.id else 
            1 if "flash" in x.id else 
            2
        ))
