...
class MemoryService:
    ...
    async def get_neural_context(self, label: str) -> Optional[Dict[str, Any]]:
        """
        Call RPC 'get_neural_context' using the node label (finding ID first).
        """
        client = self.client
        if client is None:
            return None
        ...
        try:
            # 1. Get Node ID from Label
            node_res = client.table("nodes").select("id").eq("label", label).execute()
            ...
            # 2. Call RPC
            response = client.rpc('get_neural_context', {'query_node_id': node_id}).execute()
            return response.data
        except Exception as e:
            logger.error(f"MemoryService: get_neural_context failed for {label}: {e}")
            return None
