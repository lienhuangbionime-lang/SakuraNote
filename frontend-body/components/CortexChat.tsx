...
export const CortexChat = () => {
    ...
    useEffect(() => {
        ...
        if (savedModel) {
            // [Hotfix] Reset if model is the deprecated/broken 2.0-pro-exp
            if (savedModel.includes('pro-exp-02-05')) {
                console.warn('Resetting deprecated model:', savedModel);
                setSelectedModel('models/gemini-2.5-flash');
                localStorage.setItem('CORTEX_MODEL', 'models/gemini-2.5-flash');
            } else {
    ...
    const refreshModels = async () => {
        ...
        try {
            ...
            if (data.models && data.models.length > 0) {
                ...
                localStorage.setItem('CORTEX_MODELS_UPDATED', new Date().getTime().toString());
            }
        } catch (e) {
    ...
    }, [messages, isOpen, activeTab, urlContext]); // Scroll when URL preview appears

    // Auto-detect URL in input
        ...
        if (match && !urlContext && !isAnalyzingUrl) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    setUrlContext(data);
                    // Optional: Remove URL from input to keep it clean? 
                    // setInput(text.replace(url, '').trim());
                }
            } catch (e) {
    ...
    const handleSend = async () => {
        ...
        const userMsg = input;
        const currentUrlContext = urlContext; // Capture current context

        setInput('');
        setUrlContext(null); // Clear context after sending
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/api/v1/chat/message`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: userMsg, // Send original input
                    history: currentHistory,
                    model: selectedModel,
                    url_context: currentUrlContext // Pass the rich context
                })
            });
                            ...
                            >
                                {availableModels.length > 0 ? (
                                    ...
                                    <>
                                        ...
                                        <option value="models/gemini-2.5-pro">Gemini 2.5 Pro (Smart)</option>
